...
export class RankingChartComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    const rankingValues: Map<string, number> = (this.rankingData as Map<
      RankValueEnum,
      Map<string, number>
    >).get(this.rankValue) as Map<string, number>;
    ...
    const chartData: ChartData = {
      datasets: [
        ...Array.from(rankingValues).map((c) =>
          this.generateChartDataSet(c[0], c[1])
        ),
      ],
    };
